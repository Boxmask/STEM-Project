...
viewCamera.onclick = (e) => {
    if (isTutorialActive) return; // 튜토리얼 중 배경 클릭 방지
...
window.nextTutorialStep = () => {
    if (tutPhase === 'INIT') {
        ...
        if (tutIndex < initialSteps.length) {
            ...
            tutPhase = 'WAIT_SPAWN';
            hideTut();
            triggerRandomAttack(); // 여기서 실제 엔진의 첫 공격이 시작됨
        }
