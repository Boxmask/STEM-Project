...
// 기존 threats 배열에 'RPG GUNNER' 추가
threats.push('RPG GUNNER');

function spawnAndAttackWithRPG() {
    // 1. RPG 사수 스폰 (화면 하단 지면에 랜덤 스폰)
    let posX_gunner = 20 + Math.random() * 50; // Left: 20% ~ 70%
    let posY_gunner = 'auto'; // bottom: 10% (CSS에서 설정)
    
    // 사수 초기 위치 및 크기 설정
    rpgGunnerBox.style.left = posX_gunner + '%';
    rpgGunnerBox.style.display = 'block';
    rpgGunnerLabel.innerText = 'ID: UNKNOWN';
    rpgGunnerBox.style.width = '30px'; rpgGunnerBox.style.height = '60px'; // 초기 작은 크기

    // 식별 단계 (0.5초 뒤)
    setTimeout(() => {
        rpgGunnerLabel.innerText = 'ID: RPG GUNNER (HOSTILE)';
        rpgGunnerBox.style.width = '60px'; rpgGunnerBox.style.height = '120px'; // 최종 크기
    }, 500);

    // 공격 시퀀스 (2.5초 뒤)
    setTimeout(() => {
        // RPG 사수의 총구 위치 계산 (이미지 비율 고려)
        let rect_gunner = rpgGunnerBox.getBoundingClientRect();
        let posX_launch = rect_gunner.left + (rect_gunner.width * 0.5); // 총구는 중간
        let posY_launch = rect_gunner.bottom - (rect_gunner.height * 0.3); // 총구는 하단에서 30% 위
        
        // 탄두 스폰 및 추적 박스 생성
        rpgProjectileBox.style.left = (posX_launch / viewCamera.offsetWidth * 100) + '%';
        rpgProjectileBox.style.top = (posY_launch / viewCamera.offsetHeight * 100) + '%';
        rpgProjectileBox.style.display = 'block';

        // 탄두 이동 로직 (매우 빠른 직접 궤적)
        // 화면 가장자리(`-10%`, `110%`)를 확인하여 정리하는 기존 로직 대신,
        // 탄두가 화면에 도달하는 것을 기준으로 새로운 로직을 사용합니다.
        let targetX = 50 + (Math.random() - 0.5) * 5; // Zontik-1 카메라 방향으로 약간 랜덤한 타겟
        let targetY = 50 + (Math.random() - 0.5) * 5;
        let dx = (targetX - posX_launch / viewCamera.offsetWidth * 100) / 10; // 빠른 이동
        let dy = (targetY - posY_launch / viewCamera.offsetHeight * 100) / 10;

        let posX_projectile = posX_launch / viewCamera.offsetWidth * 100;
        let posY_projectile = posY_launch / viewCamera.offsetHeight * 100;

        const projectileMoveInterval = setInterval(() => {
            posX_projectile += dx;
            posY_projectile += dy;
            rpgProjectileBox.style.left = posX_projectile + '%';
            rpgProjectileBox.style.top = posY_projectile + '%';

            // 탄두가 화면에 도달하면 (카메라 근처) 발사 시퀀스 종료 및 정리
            if (posX_projectile < -10 || posX_projectile > 110 || posY_projectile < -10 || posY_projectile > 110) {
                clearInterval(projectileMoveInterval);
                rpgProjectileBox.style.display = 'none';
                rpgGunnerBox.style.display = 'none'; // 병사도 없어짐
                
                // 3초 뒤 새로운 타겟 생성 (병사 또는 UAV 랜덤)
                setTimeout(() => {
                    if (Math.random() > 0.3) { // 70% 확률로 병사
                        spawnAndAttackWithRPG();
                    } else { // 30% 확률로 UAV
                        spawnAndTrackTarget(); // 기존 로직 사용 (UAV 이미지로 대체)
                    }
                }, 3000); 
            }
        }, 15); // 탄두 프레임 속도 (더 빠름)
        ...
        let logHtml = `<div class="log-entry">
            <span class="log-time">[${timeStr}]</span>
            <span class="log-alert">WARNING: RPG 사수 위협 식별! RPG 발사됨!</span><br>
            <span>AZIMUTH: ${randomAzimuth}° | 발사된 RPG 탄두: 추적 중...</span><br>
            <span>INTERCEPTOR DEPLOYED...</span>`;
        logContent.innerHTML = logHtml + logContent.innerHTML;

    }, 2500); // 병사 스폰 후 2.5초 뒤 발사

}

// 기존 로그 업데이트 로직 수정 (병사 공격 이벤트 통합)
setInterval(() => {
    // ... 기존 로그 코드 ...
    
    // RPG 사수 공격 이벤트가 발생하면, 로그를 더 구체적으로 업데이트합니다.
    if (Math.random() > 0.8 && cameraModeVal.innerText === 'OPTICAL') { // 20% 확률로 RPG 사수 공격
        spawnAndAttackWithRPG();
        // 탄두 이동 로직에서 로그를 추가하므로, 여기서는 추가하지 않습니다.
    } else if (Math.random() > 0.8) { // UAV 공격
        spawnAndTrackTarget();
        // ... 기존 로그 ...
    }
}, 3500);
